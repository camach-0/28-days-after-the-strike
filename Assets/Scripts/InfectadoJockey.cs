...
public class EstadoMontadoJockey : IEstadoZombi
{
    ...
    public void Entrar(ZombiController zombi)
    {
        ...
        // Ignorar colisiones entre el Jockey y el jugador
        ...
        // 🔴 El Jockey se vuelve hijo del jugador y se sobrepone encima
        zombi.transform.SetParent(jockey.jugadorMontado);
        zombi.transform.localPosition = new Vector3(0f, 0.25f, 0f); // posición relativa encima

        // Avisar al StunController del jugador → redirigir movimiento y vibrar cámara
        ...
        zombi.agente.enabled = false; // el Jockey ya no navega por sí mismo
    }
    ...
    private void TerminarMontura(ZombiController zombi, InfectadoJockey jockey)
    {
        if (jockey.jugadorMontado != null)
        {
            ...
            // 🔴 El Jockey se des-emparenta del jugador
